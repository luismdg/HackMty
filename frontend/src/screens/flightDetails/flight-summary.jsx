...
export function FlightSummary({ onFlightSelect }) {
    ...
    useEffect(() => {
        async function fetchFlights() {
            try {
                ...
                const flightsArray = Object.entries(allFlights).map(([flightId, flightData]) => ({
                    id: flightId,
                    ...
                    status: "ACTIVO",
                    country: "USA",
                    type: flightData.origin.includes("USA") && flightData.destination.includes("USA")
                        ? "Domestic"
                        : "International",
                    ...flightData
                }))
    ...
    const filteredFlights = flights.filter((flight) => {
        ...
        const matchesDuration =
            ...
            (durationFilter === "medium" && flight.duration > 3 && flight.duration <= 6) ||
            (durationFilter === "long" && flight.duration > 6)
        return matchesSearch && matchesAirline && matchesCountry && matchesDuration
    })

    if (loading) return <div className="p-8 text-white">✈️ Cargando información de vuelos...</div>
    if (error)
        return (
            <div className="p-8 text-white">
                <div className="bg-[#1A2639] border border-[#2C3E50] p-4 mb-4">
                    ...
                    <p className="text-red-500 text-sm mt-2">Mostrando datos de ejemplo...</p>
                </div>
            </div>
        )
    ...
    return (
        <div className="min-h-screen bg-[#050B16] text-white p-8 space-y-6">
            <div>
                <h1 className="text-3xl font-semibold tracking-tight text-[#C8D6E5]">Resumen de Vuelos</h1>
                <p className="text-[#7F8FA6] mt-2">Explora y filtra los vuelos disponibles.</p>
            </div>

            <div className="border border-[#1E293B] bg-[#0C1526] p-6 space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div>
                        <label className="text-xs mb-1.5 block text-[#94A3B8]">Buscar vuelo</label>
                        <input
                            placeholder="AA1234 o American Airlines"
                            value={searchTerm}
                            onChange={(e) => setSearchTerm(e.target.value)}
                            className="w-full bg-[#09111E] border border-[#1E293B] px-3 py-2 text-sm text-white placeholder:text-gray-500 focus:ring-1 focus:ring-[#3B82F6] outline-none"
                        />
                    </div>
                    <div>
                    ...
                    <div>
                        <label className="text-xs mb-1.5 block text-[#94A3B8]">Duración</label>
            ...
            <div className="overflow-hidden border border-[#1E293B]">
                <table className="w-full text-left text-sm border-collapse">
                    <thead className="bg-[#0D1B2A] border-b border-[#1E293B] text-[#94A3B8] uppercase text-xs">
                        <tr>
                    ...
                    <tbody>
                        {filteredFlights.map((flight, i) => (
                            <tr
                                ...
                                className="cursor-pointer border-b border-[#1E293B]/80 hover:bg-[#1E293B]/40 transition-colors"
                            >
